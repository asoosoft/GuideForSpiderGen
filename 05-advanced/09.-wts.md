...
{% hint style="info" %}
아래 내용에는 RestAPI 를 QueryManager로 송수신하기 위한 내용은 들어있지 않으니 해당 내용이 필요하면 샘플 소스를 참고해주세요.
{% endhint %}
...
{% hint style="info" %}
Query 시스템 사용시 위의 RestAPI, Websocket 을  단독으로 사용할 때와는다르게 컴포넌트에 필드를 매핑할 수 있습니다.&#x20;
{% endhint %}

프로젝트 트리뷰에서 Framework > sys 우클릭 > Default Load Settings.. 설정창 열고\
Library > AQuery.js, AQueryData.js, QueryManager.js 선택하여 기본 로드처리
...
class RealQueryManager extends QueryManager
{
     ...
     */
    async registerReal(aquery, realField, keyArr, compArr, updateTypes, callback, afterUpdate)
    {
        await super.registerReal.apply(this, arguments);
        
        //[조회 후 실시간 등록] 대신 [실시간 등록]만 해도 현재위데이터를표현하기해 임시로 처리
        keyArr.forEach(cd => {
            if(this.realDataObj[aquery]) {
                if(this.realDataObj[aquery][cd]) this.realProcess(this.realDataObj[aquery][cd])
            }
        })
    }
        ...
        if(isSet) {
            ...
            regArr.forEach(item => {
                if(this.realTypeMap[qryName].includes(item)) {
                    if(this.realDataObj[qryName][item]) this.realProcess(this.realDataObj[qryName][item]);
                } else {
                    if(!this.realDataObj[qryName]) this.realDataObj[qryName] = {};

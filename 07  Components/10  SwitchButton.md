...
\
\*\*스위치 버튼 (SwitchButton)\*\* 는 사용자가 on/off 상태를 전환할 수 있는 컴포넌트입니다. >SwitchButton 요소를 사용하여 클릭이 가능한 스위치 버튼(SwitchButton) 을 만듭니다. 사용자가 스위치 버튼(SwitchButton) 을 클릭하면 on/off 상태에 설정해둔 이벤트가 트리거 됩니다.
